<?php
/**
 * ThemeTim Recent Product
 */
class RecentProduct_Widget extends WP_Widget {
    /**
     * Register widget with WordPress.
     */
    function __construct() {
        parent::__construct(
            'RecentProduct_Widget',
            __( 'ThemeTim Recent Product', 'text_domain' ),
            array( 'description' => __( 'Recent Product', 'text_domain' ), )
        );
    }
    /**
    ...
    public function widget( $args, $instance ) {
        ...
        <div class="recent-product-widget default-widget <?php echo $carousel_class; ?>">
            <?php echo $title ?>
            <?php echo do_shortcode('[recent_products per_page='.$product_limit.' columns='.$product_columns.']'); ?>
        </div>
        <script>
            jQuery(function(){
                /*******************************************************************************
                 * Carousel Slider
                 *******************************************************************************/
                if(jQuery('.recent-product-carousel').length){
                    jQuery('.recent-product-carousel .products').owlCarousel({
                        loop:true,
                        margin:30,
    ...
    public function form( $instance ) {
        $title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Recent Product', 'text_domain' );
...
} // class RecentProduct_Widget

// register RecentProduct_Widget widget
function regsiter_recentproduct_widget() {
    register_widget( 'RecentProduct_Widget' );
}
add_action( 'widgets_init', 'regsiter_recentproduct_widget' );