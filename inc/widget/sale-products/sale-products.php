<?php
/**
 * ThemeTim Sale Products 
 */
class SaleProducts_Widget extends WP_Widget {
    /**
    ...
    function __construct() {
        parent::__construct(
            'SaleProducts_Widget',
            __( 'ThemeTim Sale Products', 'text_domain' ),
            array( 'description' => __( 'Sale Products', 'text_domain' ), )
        );
    }
    /**
    ...
    public function widget( $args, $instance ) {
        ...
        if ( $product_carousel_enable ==  '1' ) {
            $carousel_class = 'sale-product-carousel';
        }
        $title = '';
        ...
        ?>
        <div class="sale-product-widget default-widget <?php echo $carousel_class; ?>">
            <?php echo $title ?>
            <?php echo do_shortcode('[sale_products per_page='.$product_limit.' columns='.$product_columns.']'); ?>
        </div>
        <script>
            jQuery(function(){
                /*******************************************************************************
...
} // class SaleProducts_Widget

// register SaleProducts_Widget widget
function regsiter_SaleProducts_widget() {
    register_widget( 'SaleProducts_Widget' );
}
add_action( 'widgets_init', 'regsiter_SaleProducts_widget' );